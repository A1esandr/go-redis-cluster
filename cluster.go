...
func NewCluster(options *Options) (*Cluster, error) {
	...
	for i := range options.StartNodes {
		...
		err := cluster.auth(node)
		if err != nil {
			fmt.Printf("cluster node %s auth error: %s", node.address, err.Error())
			continue
		}
...
func (cluster *Cluster) auth(node *redisNode) error {
	reply, err := String(node.do("AUTH", node.password))
	if err != nil {
		return err
	}
	if strings.ToLower(reply) != "ok" {
		return errors.New("auth error not ok response: " + reply)
	}
	return nil
}
